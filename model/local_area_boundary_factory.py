...
def create_local_area_boundary_from_individual_entry(entry_of_data, header_list, dataset_descriptor):
    '''
    ...
    for i in range(len(entry_of_data_list)):
        if i == header_list.index(dataset_descriptor.expected_headers['abbreviation']):
            ...
            centroid_coordinates = map(float, entry_of_data_list[i].split(COORDINATES_SEPARATOR)[::-1])
